...
function renderNodes() {
    ...
    data.nodes.forEach(node => {
        ...
        el.innerHTML = html;
        el.addEventListener('click', (e) => { if (!draggingItem) openNodeModal(node.id); });
        el.addEventListener('mousedown', (e) => {
            if (e.button !== 0) return;
            draggingItem = { type: 'node', id: node.id };
            offset.x = e.clientX - node.x; offset.y = e.clientY - node.y;
            e.stopPropagation();
        });
...
function renderConnections() {
    ...
    data.nodes.forEach(source => {
        data.nodes.forEach(target => {
            ...
            // If there is ANY overlap between source outputs and target prereqs, draw a line
            const hasMatch = srcOuts.some(outId => tgtIns.includes(outId));
...
document.getElementById('workspace-graph').addEventListener('contextmenu', (e) => {
    ...
    data.nodes.push({
        id: generateId(), x: e.clientX - rect.left - 90, y: e.clientY - rect.top - 40,
