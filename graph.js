...
function isFilterEmpty(f) { return !f || (f.people.length === 0 && f.items.length === 0 && f.nodes.length === 0); }
...
function nodeMatchesFilter(node, f) {
    ...
    const nodeItems = new Set([ ...(node.prereqIds || []), ...(node.outputIds || []), ...(node.options || []).flatMap(o => [...(o.prereqIds || []), ...(o.outputIds || [])]) ]);
...
const getOwnerGradient = (ownerIds) => {
    ...
    const sortedIds = [...ownerIds].sort((a, b) => (getName(null, a) || '').localeCompare(getName(null, b) || ''));
...
function renderNodes() {
    ...
    data.nodes.forEach(node => {
        ...
        if (!nodeMatchesFilter(node, window.currentGraphFilter)) el.classList.add('faded');
        ...
        const ownerNames = (node.ownerIds || []).map(id => getName(null, id)).filter(n => n !== 'Unknown').sort((a, b) => a.localeCompare(b)).join(', ') || 'Unassigned';
        ...
        el.addEventListener('mousedown', (e) => {
            ...
            if (e.shiftKey) { if (selectedNodeIds.has(node.id)) selectedNodeIds.delete(node.id); else selectedNodeIds.add(node.id); renderNodes(); } 
            else { if (!selectedNodeIds.has(node.id)) { selectedNodeIds.clear(); selectedNodeIds.add(node.id); renderNodes(); } }
            draggingItem = { type: 'node', id: node.id }; e.stopPropagation();
        });
        el.addEventListener('contextmenu', (e) => { e.preventDefault(); e.stopPropagation(); if (typeof openNodeModal === 'function') openNodeModal(node.id); });
...
function getDistanceToSegment(p, v, w) {
    ...
    return Math.hypot(p.x - (v.x + Math.max(0, Math.min(1, t)) * (w.x - v.x)), p.y - (v.y + Math.max(0, Math.min(1, t)) * (w.y - v.y)));
}
...
function drawLine(linkId, startX, startY, endX, endY, color, showArrowHead, isFaded) {
    ...
    let pathData = `M ${startX} ${startY}`; knees.forEach(k => pathData += ` L ${k.x} ${k.y}`); pathData += ` L ${endX} ${endY}`;
    const group = document.createElementNS('http://www.w3.org/2000/svg', 'g');
    if (isFaded) group.classList.add('faded');
    const visPath = document.createElementNS('http://www.w3.org/2000/svg', 'path');
    visPath.setAttribute('d', pathData); visPath.setAttribute('class', 'connection-path'); visPath.setAttribute('stroke', color); visPath.style.pointerEvents = 'none'; 
    if (showArrowHead) {
        const markerId = `arrowhead-${color.replace(/[^a-zA-Z0-9]/g, '')}`;
        if (!document.getElementById(markerId)) {
            svg.innerHTML += `<defs><marker id="${markerId}" markerWidth="10" markerHeight="7" refX="9" refY="3.5" orient="auto"><polygon points="0 0, 10 3.5, 0 7" fill="${color}" /></marker></defs>`;
        }
    ...
    hitPath.addEventListener('mousedown', (e) => {
        ...
        for (let i = 0; i < points.length - 1; i++) { const dist = getDistanceToSegment(clickP, points[i], points[i+1]); if (dist < minDist) { minDist = dist; minIndex = i; } }
        if (!data.knees[linkId]) data.knees[linkId] = []; data.knees[linkId].splice(minIndex, 0, clickP); renderConnections();
    });
    group.appendChild(visPath); group.appendChild(hitPath); svg.appendChild(group);
    ...
    knees.forEach((knee, index) => {
        ...
        visCircle.setAttribute('cx', knee.x); visCircle.setAttribute('cy', knee.y); visCircle.setAttribute('r', 4); visCircle.setAttribute('fill', color); visCircle.style.pointerEvents = 'none';
        const hitCircle = document.createElementNS('http://www.w3.org/2000/svg', 'circle');
        hitCircle.setAttribute('cx', knee.x); hitCircle.setAttribute('cy', knee.y); hitCircle.setAttribute('r', 15); hitCircle.setAttribute('fill', 'transparent'); hitCircle.style.pointerEvents = 'fill';
        kneeGroup.addEventListener('mousedown', (e) => {
            if (e.button !== 0) return; e.stopPropagation(); e.preventDefault();
            if (e.shiftKey) { data.knees[linkId].splice(index, 1); renderConnections(); } else { draggingItem = { type: 'knee', linkId: linkId, index: index }; kneeGroup.style.cursor = 'grabbing'; }
        });
        kneeGroup.appendChild(visCircle); kneeGroup.appendChild(hitCircle); group.appendChild(kneeGroup); 
    });
}
...
function renderConnections() {
    ...
    data.nodes.forEach(source => {
        data.nodes.forEach(target => {
            ...
            let srcBundles = [];
            if (!source.isDecision) srcBundles.push({ subId: 'main', ids: source.outputIds || [], startY: 40, color: 'var(--line-color)', arrowHead: true });
            else (source.options || []).forEach((opt) => {
                let optY = 40; const srcEl = document.getElementById(source.id);
                if (srcEl) { const optEl = srcEl.querySelector(`.node-opt-row[data-opt-id="${opt.id}"]`); if (optEl) optY = optEl.offsetTop + (optEl.offsetHeight / 2); }
                srcBundles.push({ subId: opt.id, ids: opt.outputIds || [], startY: optY, color: 'var(--line-color)', arrowHead: false });
            });
            let tgtBundles = []; tgtBundles.push({ subId: 'main', ids: target.prereqIds || [], endY: 40 });
            if (target.isDecision) (target.options || []).forEach(opt => {
                let optY = 40; const tgtEl = document.getElementById(target.id);
                if (tgtEl) { const optEl = tgtEl.querySelector(`.node-opt-row[data-opt-id="${opt.id}"]`); if (optEl) optY = optEl.offsetTop + (optEl.offsetHeight / 2); }
...
window.addEventListener('mousemove', (e) => {
    ...
    if (draggingItem.type === 'node') {
        ...
        if (selectedNodeIds.has(draggingItem.id)) { selectedNodeIds.forEach(id => { const node = data.nodes.find(n => n.id === id); if (node) { node.x += dx; node.y += dy; } }); } 
        else { const node = data.nodes.find(n => n.id === draggingItem.id); if (node) { node.x += dx; node.y += dy; } }
...
workspaceGraph.addEventListener('wheel', (e) => {
    e.preventDefault(); const zoomIntensity = 0.1; const zoom = Math.exp((e.deltaY < 0 ? 1 : -1) * zoomIntensity);
...
document.getElementById('btn-center-fit').addEventListener('click', () => {
    if (data.nodes.length === 0) return;
    let minX = Infinity, minY = Infinity, maxX = -Infinity, maxY = -Infinity;
    data.nodes.forEach(n => { if(n.x<minX)minX=n.x; if(n.y<minY)minY=n.y; if(n.x>maxX)maxX=n.x; if(n.y>maxY)maxY=n.y; });
    const gw = (maxX - minX) + 200; const gh = (maxY - minY) + 100;
    graphScale = Math.min(1, Math.min(workspaceGraph.clientWidth / gw, workspaceGraph.clientHeight / gh) * 0.9);
    graphPanX = (workspaceGraph.clientWidth - (gw * graphScale)) / 2 - (minX * graphScale);
    graphPanY = (workspaceGraph.clientHeight - (gh * graphScale)) / 2 - (minY * graphScale);
    updateGraphTransform();
});
